...
namespace Brickweave.Cqrs
{
    public class QueryDispatcher : IQueryDispatcher
    {
        ...
        public async Task<object> ExecuteAsync(IQuery query, ClaimsPrincipal user = null, CancellationToken cancellationToken = default(CancellationToken))
        {
            ...
            if (handler is ISecured)
                return await handler.HandleAsync((dynamic)query, user, cancellationToken);

            return await handler.HandleAsync((dynamic)query, cancellationToken);
        }

        public async Task<TResult> ExecuteAsync<TResult>(IQuery<TResult> query, ClaimsPrincipal user = null, CancellationToken cancellationToken = default(CancellationToken))
        {
            Guard.AgainstNullArgument(nameof(query), query);
